...
- llama.cpp 编译、模型加载和推理等高资源命令一次只能启动一个进程；禁止在单次工具调用中用循环、并行任务或串联命令连续启动多个模型进程。
- 高资源命令必须显式设置与当前诊断相称的线程数、上下文长度和生成 token 数；这些参数只约束本次工程诊断，不得写成 APP 对用户请求的限制。
- 工具调用返回 `aborted`、客户端崩溃、会话重启或输出通道中断后，必须先检查并处理残留的 `llama`、CMake、Ninja、Gradle 和 Java 构建进程，记录上一次命令及退出状态；未确认没有残留进程前禁止原样重跑。
- 预计产生大量输出的构建和推理必须降低日志详细度，或把完整日志写入被 Git 忽略的诊断目录，再只读取末尾和错误摘要；禁止把 MSVC include trace、完整构建日志或无界模型输出直接灌入交互通道。
