...
本项目是自用开发版：不要加入任何对于导入的东西的校验和门槛。不得擅自添加面向用户的数据处理条数上限、结果截断，大小限制，长度限制，或其他的“保护性”限制，用户自己搞自己炸了，自己负责。不要把用户当巨婴。。需要控制运行资源时只能采用透明、统一的调度机制，不能丢弃用户请求、静默截断数据或把限制伪装成成功；任何确需限制的外部系统约束必须直接暴露并记录原因。
...
| **正式验证 / 交付** | 用户明确要求“正式编译”“安装”“正式回归”“完整验证闭环”或交付 | 按用户明确授权执行对应正式动作 | 仅在明确需要人工操作时参与 | 已完成的源码必须先提交 | **是，仅执行用户明确要求的部分** | 对应正式动作完成并记录结果；完整闭环为 LocalCore Release 编译 → 模拟器安装 → 复现与回归 |
...
- AI 先完成原因排查和动态方案验证所需准备，再通过 F 工具把补丁注入当前雷电模拟器中正在运行的 LocalCore 进程。
...
- 全自动模式按以下流程循环执行：自主复现 → 定位根因 → 修改源码 → 提交并推送 → 按“Windows 增量构建”规则正式编译 LocalCore Release APK → 安装到设备名包含 `emulator` 的模拟器 → 自动运行、测试和回归。若验证失败，直接进入下一轮修复，直到问题解决或确认当前方案不可行。
...
- 用户明确要求“完整验证闭环”时，执行：正式编译 LocalCore Release APK（按“Windows 增量构建”规则从 `D:\AI\LLM\LocalCore` 构建）→ 安装到设备名包含 `emulator` 的模拟器 → 复现并回归验证。
- 所有正式动作继续遵守本文件的设备与构建约束。

#### F 工具术语与修饰指令

- **F 工具就是 Frida。** 用户说“F 工具”时按 Frida 理解。
- “用 F 工具推上去永久有效”表示：把补丁通过 Frida 注入到当前模拟器中正在运行的 LocalCore APP，并且不设置失效时间，用于持续验证方案可靠性；**不是把补丁写进源码**。
...
本节只在两种情况下生效：用户明确要求正式验证，或用户明确启用全自动模式。普通模式下只执行用户明确要求的验证动作；用户要求完整验证闭环时，执行 LocalCore Release 正式编译 → 安装到设备名包含 `emulator` 的模拟器 → 复现并回归。全自动模式下自动重复这一闭环直到问题收敛。
...
- 用户单独说“编译”，固定只执行 APK 增量编译：从 `D:\AI\LLM\LocalCore` 运行 `cmd /c yarn.cmd build:android:release`。不得自动编译核心、创建 Release、安装或回归。
- 核心编译必须由用户明确要求；即使存在核心源码、补丁、构建脚本或 ABI 改动，也不得据此把“编译 APK”扩展成核心编译。若 APK 的新功能需要新核心，只报告依赖及当前未交付状态。
- 快速编译复用现有依赖、Gradle daemon、Gradle 用户目录和构建缓存；不例行执行依赖安装、`clean`、删除 `.cxx` / build 目录、停止 daemon、切换构建路径或重建环境。只有明确错误证据才处理对应问题，不做猜测性清理。
- 每次编译按规则递增应用版本号，先提交并推送，再直接执行上述唯一 APK 构建入口。无源码变化且缓存命中时应尽量缩短耗时；版本打包、JS 打包或原生源码变化可能触发必要任务，不得承诺每次几秒完成，也不得用旧 APK 冒充新产物。
- 构建完成报告本次实际耗时、版本和 APK 路径；构建失败直接报告原因，不自动转去编译核心。
...
- 固定从 `D:\AI\LLM\LocalCore` 直接构建；native 中间目录由 `android/app/build.gradle` 外迁到 `D:/cxx/LocalCore`，并设置 `-DCMAKE_OBJECT_PATH_MAX=250`（250：在 ninja 260 上限前触发 hashing，且大于最长对象目录约 159 字符；128 会连目录都放不下）。
- 日常增量构建不要执行 `clean`；只有在有明确缓存或构建状态错误证据时才清理对应缓存。

```powershell
Set-Location D:\AI\LLM\LocalCore
cmd /c yarn.cmd build:android:release
```
...
- 禁止通过 Codex 交互工具在 Windows 主机直接启动任何会加载模型或执行推理的 llama.cpp 可执行程序，包括但不限于 `llama-completion`、`llama-cli`、`llama-app`、`llama-server`、benchmark、测试程序以及它们的重命名、复制或脚本包装形式。该禁令不因模型大小、量化格式、线程数、上下文长度、生成 token 数、日志级别、输出重定向或单进程运行而放宽；不得再通过缩小参数或隐藏输出试跑来判断是否安全。
- 推理回归统一通过目标设备上的 `NativeRuntime -> liblocalcore_loader.so -> liblocalcore_core.so` 实际链路和真实 APP/API 请求完成。确需官方主机 CLI 对照时，只能把完整命令交给用户在 Codex 交互工具之外执行，再分析用户提供的退出状态和日志；不得由 Codex 代为启动该模型进程。
