...
- 用户单独说“编译”，默认代表着用户的意图只是进行 APK 增量编译：从 `D:\AI\LLM\LocalCore` 运行 `cmd /c yarn.cmd build:android:release`。你立刻进行 APK 的增量编译，这个耗时应该不会超过一分钟。编译APK完成之后，再检查新的提交是否需要编译核心，如果需要，必须告知用户，本次改动需要编译核心，告知之后，自动进行核心编译，并且把核心推送到 release。
